...
class ExportWordPage(ctk.CTkFrame):
    ...
    def export_to_word(self):
        ...
        progress_frame = ctk.CTkFrame(progress_win, fg_color="transparent")
        ...
        # File counter label
        file_counter = ctk.CTkLabel(
            progress_frame,
            text="Processing...",
            font=ctk.CTkFont(size=12),
            text_color="#ffffff"
        )
        file_counter.pack(pady=(5, 0))
        ...
        def animate_progress():
            if not getattr(self, '_progress_animating', False):
                return
            current = getattr(progress_bar, '_current_value', 0)
            next_val = current + 0.03
            if next_val > 1:
                next_val = 0
            progress_bar.set(next_val)
            progress_win.after(30, animate_progress)
        animate_progress()
        ...
        def do_export():
            ...
            try:
                ...
                self.after(0, lambda: self.status_label.configure(text="✅ Export completed successfully!", text_color="green"))
            except Exception as e:
                ...
                self.after(0, lambda: (progress_win.grab_release(), progress_win.destroy()))
                    ...
                    if out_path and os.path.exists(out_path):
                        ...
                        if save_path:
                            try:
                                ...
                                self.after(0, lambda: self.status_label.configure(text=f"❌ Error saving file: {str(e)}", text_color="red"))
                                continue
