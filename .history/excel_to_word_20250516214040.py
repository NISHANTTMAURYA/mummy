...
def process_single_month(template_path, month_name, data_rows, columns, file_info, output_path):
    ...
    try:
        ...
        if not target_table:
            ...
            return False
        # Find column indices for XI and XII
        header_cells = target_table.rows[1].cells if len(target_table.rows) > 1 else []
        xi_cols = {'ALLOTTED': None, 'ENGAGED': None, 'GAP': None}
        xii_cols = {'ALLOTTED': None, 'ENGAGED': None, 'GAP': None}
        for idx, cell in enumerate(header_cells):
            txt = cell.text.strip().upper()
            if txt == 'ALLOTTED' and idx < 5:
                xi_cols['ALLOTTED'] = idx
            elif txt == 'ENGAGED' and idx < 5:
                xi_cols['ENGAGED'] = idx
            elif txt == 'GAP' and idx < 5:
                xi_cols['GAP'] = idx
            elif txt == 'ALLOTTED' and idx >= 5:
