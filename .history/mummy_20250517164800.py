...
class ExcelPage(ctk.CTkFrame):
    ...
    def open_excel_file(self):
        excel_path = resource_path("iso_excel.xlsx")
...
class EditPage(ctk.CTkFrame):
    ...
    def get_file_list(self):
        ...
        excel_copies_path = resource_path("excel_copies")
        if not os.path.exists(excel_copies_path):
            return []
        # Get only valid Excel files, filtering out system files and temp files
        all_files = os.listdir(excel_copies_path)
    ...
    def on_file_change(self, filename):
        import openpyxl
        self.current_file = os.path.join(resource_path("excel_copies"), filename)
...
class CopyPage(ctk.CTkFrame):
    ...
    def create_copy(self):
        ...
        excel_copies_path = resource_path("excel_copies")
        os.makedirs(excel_copies_path, exist_ok=True)
        new_file = os.path.join(excel_copies_path, f"iso_excel_{year}_{term}_{std}.xlsx")
        
        try:
            import openpyxl
            
            # Open the source file
            src_wb = openpyxl.load_workbook(resource_path("iso_excel.xlsx"))
    ...
    def refresh_file_list(self):
        ...
        excel_copies_path = resource_path("excel_copies")
        if os.path.exists(excel_copies_path):
            # Get only valid Excel files, filtering out system files and temp files
            all_files = os.listdir(excel_copies_path)
    ...
    def _display_filtered_files(self):
        ...
        if not filtered_files:
            ...
            for i, (fname, info) in enumerate(sorted(filtered_files), start=0):
                file_path = os.path.abspath(os.path.join(resource_path("excel_copies"), fname))
...
class App(ctk.CTk):
    def __init__(self):
        ...
        # Add Developer Tools button
        self.dev_tools_button = ctk.CTkButton(
            button_frame,
        ...
        self.dev_tools_button.grid(row=4, column=0, padx=10, pady=10, sticky="ew")
        ...
        self.pages = {
            ...
            "dev_tools": DevToolsPage(self.content_frame)
        }
    ...
    def show_dev_tools(self):
        self._show_page("dev_tools")
        self.dev_tools_button.configure(fg_color=self.cute_colors["accent"])
        self.copy_button.configure(fg_color=self.cute_colors["button_bg"])
        self.edit_button.configure(fg_color=self.cute_colors["button_bg"])
        self.export_button.configure(fg_color=self.cute_colors["button_bg"])
        self.merge_terms_button.configure(fg_color=self.cute_colors["button_bg"])
...
class ExportWordPage(ctk.CTkFrame):
    ...
    def refresh_file_list(self):
        ...
        excel_files = [f for f in os.listdir(resource_path("excel_copies")) if f.endswith(".xlsx")]
        if not excel_files:
            ctk.CTkLabel(
                files_container,
        ...
        compatible_pairs = []
        single_files = []
        file_ctimes = {f: os.path.getctime(os.path.join(resource_path("excel_copies"), f)) for f in excel_files}
                ...
                try:
                    ...
                    for i, file1 in enumerate(selected_files):
                        ...
                        for file2 in selected_files[i+1:]:
                            ...
                            file2_info = self._parse_filename(file2)
                            if self._are_files_compatible(file1_info, file2_info):
                                excel_path1 = os.path.join(resource_path("excel_copies"), file1)
                                excel_path2 = os.path.join(resource_path("excel_copies"), file2)
                                ...
                                if file1 not in processed_files:
                                    excel_path = os.path.join(resource_path("excel_copies"), file1)
                    ...
                    for file in selected_files:
                        if file not in processed_files:
                            excel_path = os.path.join(resource_path("excel_copies"), file)
                            out_path = process_single_excel_file(excel_path)
                            if out_path:
                                generated_files.append(out_path)
    ...
    def _open_single_file(self, filename):
        ...
        file_path = os.path.join(resource_path("excel_copies"), filename)
        webbrowser.open(f"file://{os.path.abspath(file_path)}")

    def _open_file_pair(self, file1, file2):
        """Open both files in a compatible pair"""
        file1_path = os.path.join(resource_path("excel_copies"), file1)
        file2_path = os.path.join(resource_path("excel_copies"), file2)
...
class MergeTermsPage(ctk.CTkFrame):
    ...
    def get_years(self):
        ...
        years = set()
        output_word_files_path = resource_path("output_word_files")
        if os.path.exists(output_word_files_path):
            files = os.listdir(output_word_files_path)
            term1_years = set()
            term2_years = set()
            for fname in files:
    ...
    def merge_terms(self):
        ...
        path1 = os.path.join(resource_path("output_word_files"), file1)
        path2 = os.path.join(resource_path("output_word_files"), file2)
        ...
        merged_path = os.path.join(resource_path("output_word_files"), merged_name)
...
class DevToolsPage(ctk.CTkFrame):
    def __init__(self, parent):
        super().__init__(parent)
        self.grid_columnconfigure(0, weight=1)
        self.grid_rowconfigure(0, weight=1)
        ...
        ctk.CTkLabel(
            container,
            text="Manage files in bundled folders",
            font=ctk.CTkFont(size=18, family="Arial"),
            text_color="#ffffff"
        ).grid(row=1, column=0, pady=(0, 20))
        
        # Folder selection
        folder_frame = ctk.CTkFrame(container, fg_color=self.colors["bg_secondary"], corner_radius=15)
        folder_frame.grid(row=2, column=0, sticky="ew", pady=(0, 20), padx=20)
        ...
        self.folder_var = ctk.StringVar(value="excel_copies")
        self.folder_menu = ctk.CTkOptionMenu(
            folder_frame,
            variable=self.folder_var,
            values=["excel_copies", "output_word_files"],
        ...
        self.folder_menu.grid(row=0, column=1, padx=20, pady=20, sticky="w")
        
        # File list
        list_frame = ctk.CTkFrame(container, fg_color=self.colors["bg_secondary"], corner_radius=15)
        list_frame.grid(row=3, column=0, sticky="nsew", pady=(0, 20), padx=20)
        list_frame.grid_columnconfigure(0, weight=1)
    ...
    def refresh_file_list(self, *args):
        ...
        if not os.path.exists(folder_path):
            ctk.CTkLabel(
                ...
                text=f"Folder {folder} not found",
                font=ctk.CTkFont(size=14),
                text_color="#ffffff"
            ).grid(row=0, column=0, pady=20)
            return
        ...
        for i, file in enumerate(files):
            row_frame = ctk.CTkFrame(self.scrollable_frame, fg_color=self.colors["file_bg"], corner_radius=10)
            row_frame.grid(row=i, column=0, sticky="ew", pady=3, padx=5)
            
            # Configure row layout
            row_frame.grid_columnconfigure(0, weight=0)  # File icon
            row_frame.grid_columnconfigure(1, weight=1)  # Filename
    ...
    def delete_file(self, filename):
        folder = self.folder_var.get()
        file_path = os.path.join(resource_path(folder), filename)
        
        try:
            os.remove(file_path)
            self.status_label.configure(text=f"✅ Deleted {filename}", text_color="green")
            self.refresh_file_list()
        except Exception as e:
            self.status_label.configure(text=f"❌ Error deleting {filename}: {str(e)}", text_color="red")
