...
def process_single_month(template_path, month_name, data_rows1, data_rows2, columns1, columns2, file_info1, file_info2, output_path):
    ...
    try:
        doc = Document(resource_path(template_path))
...
def create_multi_month_document(csv_path1, csv_path2, template_path, output_folder="output_word_files"):
    ...
    try:
        ...
        with open(csv_path1, 'r', newline='', encoding='utf-8-sig') as csvfile1, open(csv_path2, 'r', newline='', encoding='utf-8-sig') as csvfile2:
            ...
            # --- DEBUG LOGGING ---
            logger.info(f"all_data1 initials: {[r[1] for r in all_data1 if len(r) > 1]}")
            logger.info(f"all_data2 initials: {[r[1] for r in all_data2 if len(r) > 1]}")
            logger.info(f"all_data1 rows: {all_data1}")
            logger.info(f"all_data2 rows: {all_data2}")
            ...
            for month_name in common_months:
                if month_name in month_field_map1 and month_name in month_field_map2:
                    # --- Ensure FYJC data goes to XI, SYJC to XII ---
                    if file1_info['original_std'] == 'FYJC':
                        fyjc_data, fyjc_cols, fyjc_info = all_data1, month_field_map1[month_name], file1_info
                        syjc_data, syjc_cols, syjc_info = all_data2, month_field_map2[month_name], file2_info
                    else:
                        fyjc_data, fyjc_cols, fyjc_info = all_data2, month_field_map2[month_name], file2_info
                        syjc_data, syjc_cols, syjc_info = all_data1, month_field_map1[month_name], file1_info
                    month_file = os.path.join(month_folder, f"{month_name}_{file1_info['year_range']}.docx")
                    success = process_single_month(
                        template_path=template_path,
                        month_name=month_name,
                        data_rows1=fyjc_data,
                        data_rows2=syjc_data,
                        columns1=fyjc_cols,
                        columns2=syjc_cols,
                        file_info1=fyjc_info,
                        file_info2=syjc_info,
...
def process_excel_files(excel_folder="excel_copies", template_path="executive_summary_template.docx", output_folder="output_word_files"):
    ...
    excel_folder = resource_path(excel_folder)
    ...
    for filename in os.listdir(excel_folder):
        if filename.endswith(('.xlsx', '.xls')) and filename.startswith('iso_excel_'):
            ...
            if csv_path:
                try:
                    month_files = create_multi_month_document(csv_path, csv_path, template_path, output_folder)
                finally:
                    try:
                        if csv_path:
                            os.unlink(csv_path)
...
def process_single_excel_file(excel_path, template_path="executive_summary_template.docx", output_folder="output_word_files"):
    ...
    excel_path = resource_path(excel_path)
    template_path = resource_path(template_path)
    output_folder = resource_path(output_folder)
    ...
    try:
        ...
        with open(csv_path, 'r', newline='', encoding='utf-8-sig') as csvfile:
            ...
            if month_files:
                ...
                # --- Clean up the month files ---
                for f in month_files:
                    try:
                        os.remove(f)
                        logger.info(f"Cleaned up month file: {f}")
                    except Exception as e:
                        logger.error(f"Error deleting month file {f}: {e}")
...
def process_dual_excel_files(excel_path1, excel_path2, template_path="executive_summary_template.docx", output_folder="output_word_files"):
    ...
    excel_path1 = resource_path(excel_path1)
    excel_path2 = resource_path(excel_path2)
    template_path = resource_path(template_path)
    output_folder = resource_path(output_folder)
    ...
    try:
        ...
        month_files = create_multi_month_document(csv_path1, csv_path2, template_path, output_folder)
        
        # Combine the files
        combined_filename = f"COMBINED_{file1_info['year_range']}_term{file1_info['term']}_all_months.docx"
        combined_path = os.path.join(output_folder, combined_filename)
        
        # Sort month_files by month_order, put TOTAL at the end if present
        month_order = ['JUNE', 'JULY', 'AUG', 'SEP', 'OCT', 'NOV', 'DEC', 'JAN', 'FEB', 'MAR', 'APR', 'MAY']
        def month_sort_key(f):
            base = os.path.basename(f)
            name = base.split('_')[0].upper()
            if name == 'TOTAL':
                return 999
            return month_order.index(name) if name in month_order else 998
        month_files = sorted(month_files, key=month_sort_key)
        # Move TOTAL to the end if present
        total_files = [f for f in month_files if os.path.basename(f).startswith('TOTAL')]
        month_files = [f for f in month_files if not os.path.basename(f).startswith('TOTAL')] + total_files
        ...
        # --- Clean up only the used month files ---
        for f in month_files:
            try:
                os.remove(f)
            except Exception as e:
