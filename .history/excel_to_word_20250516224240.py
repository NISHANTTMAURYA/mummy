...
def process_single_month(template_path, month_name, data_rows, columns, file_info, output_path, month_field_map=None):
    ...
    try:
        ...
        # Use the columns argument directly for TOTAL (when month_name is a range like 'JUNE-OCT')
        if month_field_map and month_name.upper() in month_field_map:
            field_map = month_field_map[month_name.upper()]
        else:
            field_map = columns
