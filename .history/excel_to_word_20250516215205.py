...
def get_placeholder_col_map(table):
    """Return a dict mapping placeholder name to column index from the first row of the table."""
    placeholder_map = {}
    first_row = table.rows[0]
    for idx, cell in enumerate(first_row.cells):
        txt = cell.text.strip()
        if txt.startswith('{{col_') and txt.endswith('}}'):
            placeholder_map[txt] = idx
    logger.info(f"Table placeholder column map: {placeholder_map}")
...
def process_single_month(template_path, month_name, data_rows, columns, file_info, output_path):
    ...
    try:
        ...
        for table in doc.tables:
            if len(table.rows) > 0:
                for cell in table.rows[0].cells:
                    if cell.text.strip().startswith('{{col_'):
                        target_table = table
                        break
        ...
        if not target_table:
            ...
            return False
        # Get placeholder column map
        col_map = get_placeholder_col_map(target_table)
        # Remove the placeholder row after mapping
        target_table._tbl.remove(target_table.rows[0]._tr)
        # Map CSV columns to placeholders
        csv_map = {
            'SR.NO.': '{{col_srno}}',
            'INITIALS': '{{col_initials}}',
            'ALLOTTED_XI': '{{col_xi_allotted}}',
            'E-Act_XI': '{{col_xi_engaged}}',
            'E-Add_XI': '{{col_xi_gap}}',
            'ALLOTTED_XII': '{{col_xii_allotted}}',
            'E-Act_XII': '{{col_xii_engaged}}',
            'E-Add_XII': '{{col_xii_gap}}',
        }
        # For SYJC, fill XII columns, for FYJC, fill XI columns
        is_syjc = file_info['original_std'] == 'SYJC'
        is_fyjc = file_info['original_std'] == 'FYJC'
        # Data starts after header rows (now that placeholder row is removed)
        first_data_row = 1
        for data_idx, data_row in enumerate(data_rows):
            ...
            if is_fyjc or not is_syjc:
                if col_map.get('{{col_xi_allotted}}') is not None and 'ALLOTTED' in columns and len(data_row) > columns['ALLOTTED']:
                    row.cells[col_map['{{col_xi_allotted}}']].text = data_row[columns['ALLOTTED']]
                else:
                    row.cells[col_map['{{col_xi_allotted}}']].text = '--'
                if col_map.get('{{col_xi_engaged}}') is not None and 'ENGAGED' in columns and len(data_row) > columns['ENGAGED']:
                    row.cells[col_map['{{col_xi_engaged}}']].text = data_row[columns['ENGAGED']]
                else:
                    row.cells[col_map['{{col_xi_engaged}}']].text = '--'
                if col_map.get('{{col_xi_gap}}') is not None and 'GAP' in columns and len(data_row) > columns['GAP']:
                    val = data_row[columns['GAP']]
            ...
            if is_syjc or not is_fyjc:
                if col_map.get('{{col_xii_allotted}}') is not None and 'ALLOTTED' in columns and len(data_row) > columns['ALLOTTED']:
                    row.cells[col_map['{{col_xii_allotted}}']].text = data_row[columns['ALLOTTED']]
                else:
                    row.cells[col_map['{{col_xii_allotted}}']].text = '--'
                if col_map.get('{{col_xii_engaged}}') is not None and 'ENGAGED' in columns and len(data_row) > columns['ENGAGED']:
                    row.cells[col_map['{{col_xii_engaged}}']].text = data_row[columns['ENGAGED']]
                else:
                    row.cells[col_map['{{col_xii_engaged}}']].text = '--'
                if col_map.get('{{col_xii_gap}}') is not None and 'GAP' in columns and len(data_row) > columns['GAP']:
                    val = data_row[columns['GAP']]
                    if val and not val.startswith('+'):
                        val = f'+{val}'
