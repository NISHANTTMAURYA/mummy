...
class EditPage(ctk.CTkFrame):
    ...
    def _convert_to_number(self, value, header):
        """Convert a value to a number, handling special cases."""
        if not value or str(value).strip() == '':
            return None
            
        # For E-Add values, keep the '+' prefix but ensure the number part is numeric
        if header == "E-Add":
            v = str(value).strip().replace('+', '').strip()
            if v == '':
                return '+'
            try:
                num = float(v)
        ...
        # For other headers, convert to number
        try:
            return float(value)
        except ValueError:
            return None
    ...
    def _save_single_cell(self, item_id, col_idx, header, value):
        ...
        try:
            ...
            cell_value = self._convert_to_number(value, header)
        ...
        try:
            ...
            for row_idx, values in zip(row_indices, current_values):
                for i, col in enumerate(header_indices):
                    ...
                    # Convert value to number
                    cell_value = self._convert_to_number(val, header)
