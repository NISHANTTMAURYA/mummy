...
def parse_month_field_columns(header_row, field_row):
    ...
    month_field_map = {}
    current_month = None
    for idx, (month_cell, field_cell) in enumerate(zip(header_row, field_row)):
        month = month_cell.strip().upper()
        field = field_cell.strip().upper()
        if month in ['JUNE', 'JULY', 'AUG', 'SEP', 'OCT', 'NOV', 'DEC', 'JAN', 'FEB', 'MAR', 'APR', 'MAY']:
            current_month = month
            if current_month not in month_field_map:
                month_field_map[current_month] = {}
        if current_month and field in ['ALLOTTED', 'ENGAGED', 'GAP']:
            month_field_map[current_month][field] = idx
