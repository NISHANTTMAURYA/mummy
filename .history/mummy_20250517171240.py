...
def resource_path(relative_path):
    """ Get absolute path to resource, works for dev and for PyInstaller, always inside _internal """
    ...
    abs_path = os.path.join(internal_path, relative_path)
    # If it's a folder, create it if it doesn't exist
    if relative_path in ["excel_copies", "output_word_files"]:
        if not os.path.exists(abs_path):
            os.makedirs(abs_path)
    return abs_path

def ensure_required_files():
    """Ensure all required files exist in _internal folder by extracting them from executable"""
    required_files = [
        "iso_excel.xlsx",
        "executive_summary_template.docx"
    ]
    
    # Get base path for both dev and frozen environments
    if getattr(sys, 'frozen', False):
        base_path = os.path.dirname(sys.executable)
        # Path to bundled files inside the executable
        bundle_dir = getattr(sys, '_MEIPASS', os.path.dirname(sys.executable))
    else:
        base_path = os.path.dirname(os.path.abspath(__file__))
        bundle_dir = base_path
    
    # Ensure _internal directory exists
    ...
    for filename in required_files:
        ...
        # If file doesn't exist in _internal
        if not os.path.exists(internal_file):
            # Try to copy from bundled directory first (works for --onefile)
            bundled_file = os.path.join(bundle_dir, filename)
            if os.path.exists(bundled_file):
                shutil.copy2(bundled_file, internal_file)
                print(f"Copied {filename} from bundle to _internal")
                continue
                
            # If not in bundle, try the executable directory (works for --onedir)
            exe_dir_file = os.path.join(base_path, filename)
            if os.path.exists(exe_dir_file):
                shutil.copy2(exe_dir_file, internal_file)
                print(f"Copied {filename} from exe directory to _internal")
                continue
                
            # If all else fails, extract from backup inside code (as a last resort)
            # This method would require embedding the files or having an extraction method
            print(f"WARNING: Could not find {filename} - functionality may be limited")
