...
def process_single_month(template_path, month_name, data_rows, columns, file_info, output_path):
    ...
    try:
        ...
        if not target_table:
            ...
            return False
        xi_cols, xii_cols = find_standard_columns(target_table)
        # Find the first data row (skip header rows)
        first_data_row = 0
        for i, row in enumerate(target_table.rows):
            if all(cell.text.strip() == '' for cell in row.cells):
                continue
            # Heuristic: first row with SR.NO. or 1 in first cell is data
            if row.cells[0].text.strip().isdigit() or row.cells[0].text.strip().upper() == '1':
                first_data_row = i
                break
        logger.info(f"First data row index in table: {first_data_row}")
        # Fill data rows
        for data_idx, data_row in enumerate(data_rows):
            ...
            logger.info(f"Filling table row {table_row_idx}: SR.NO.={data_row[0].strip()}, INITIALS={data_row[1].strip() if len(data_row)>1 else ''}")
            row.cells[0].text = data_row[0].strip()
            row.cells[1].text = data_row[1].strip() if len(data_row) > 1 else ''
            # XI columns
            for key, idx in xi_cols.items():
                if idx is not None:
                    if file_info['original_std'] == 'FYJC' and key in columns and len(data_row) > columns[key]:
                        val = data_row[columns[key]]
                        if key == 'GAP' and val and not val.startswith('+'):
                            val = f'+{val}'
                        logger.info(f"  XI {key} (col {idx}): {val}")
                        row.cells[idx].text = val
                    else:
                        logger.info(f"  XI {key} (col {idx}): -- (no data)")
                        row.cells[idx].text = '--'
            # XII columns
            for key, idx in xii_cols.items():
                if idx is not None:
                    if file_info['original_std'] == 'SYJC' and key in columns and len(data_row) > columns[key]:
                        val = data_row[columns[key]]
                        if key == 'GAP' and val and not val.startswith('+'):
                            val = f'+{val}'
                        logger.info(f"  XII {key} (col {idx}): {val}")
                        row.cells[idx].text = val
                    else:
                        logger.info(f"  XII {key} (col {idx}): -- (no data)")
                        row.cells[idx].text = '--'
