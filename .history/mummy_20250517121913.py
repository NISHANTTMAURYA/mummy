...
class ExportWordPage(ctk.CTkFrame):
    ...
    def export_to_word(self):
        ...
        try:
            ...
            os.makedirs("output_word_files", exist_ok=True)
            ...
            # If only one file selected, process it directly
            if len(selected_files) == 1:
                excel_path = os.path.join("excel_copies", selected_files[0])
                process_single_excel_file(excel_path)
            else:
                # For multiple files, ask user if they want to merge compatible files
                file1_info = self._parse_filename(selected_files[0])
                file2_info = self._parse_filename(selected_files[1])
                
                if self._are_files_compatible(file1_info, file2_info):
                    # Show dialog asking if user wants to merge
                    dialog = ctk.CTkInputDialog(
                        text="These files are compatible. Would you like to merge them?\n\n" +
                             f"1. {selected_files[0]}\n" +
                             f"2. {selected_files[1]}\n\n" +
                             "Enter 'yes' to merge, 'no' to process separately:",
                        title="Merge Compatible Files"
                    )
                    result = dialog.get_input()
                    
                    if result and result.lower() == 'yes':
                        # Process as a pair
                        excel_path1 = os.path.join("excel_copies", selected_files[0])
                        excel_path2 = os.path.join("excel_copies", selected_files[1])
                    ...
                    # Files not compatible, process individually
                    for file in selected_files:
                        excel_path = os.path.join("excel_copies", file)
                        process_single_excel_file(excel_path)
            ...
            output_path = os.path.abspath("output_word_files")
