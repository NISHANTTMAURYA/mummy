...
def find_standard_columns(table):
    ...
    # Search all header rows for XI and XII
    for row in table.rows[:3]:  # Check first 3 rows for headers
        for idx, cell in enumerate(row.cells):
            ...
            if txt == 'XI':
                ...
                xii_start = idx
    # Now, in the row below XI/XII, find sub-columns
...
def process_single_month(template_path, month_name, data_rows, columns, file_info, output_path):
    ...
    try:
        ...
        if not target_table:
            ...
            return False
        # Dynamically find columns for XI and XII
        xi_cols, xii_cols = find_standard_columns(target_table)
        row_idx = 2  # Data starts after header rows
